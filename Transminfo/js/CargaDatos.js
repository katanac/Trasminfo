  ...
  autosaveInterval: 4000
    ...
    $.ajax({
      ...
      function verVagones(dato_nombreEstacion){
        ...
        $("#rutapop").empty();
        let countVagon = 0;
        $("#rutapop").append(dato_nombreEstacion+'<br><br>');
    ...
    function mover()	{
      ...
      $("#rutapop").css({top: coorY -570 , left: coorX -290, position:'absolute'});
      document.getElementById("rutapop").style.visibility = "visible";
    }
    function VerRut(station){
        detallesEstacion(station.title);
        $("#rutapop").parent().css({position: 'relative'});
        $("#rutapop").css({top: coorY -470 , left: coorX -290, position:'absolute'});
        document.getElementById('rutapop').style.visibility="visible";        
    }
    function ocultar()	{
